/**
 * Half-time trivia question generation from first-half match events.
 */
...
export interface GeneratedQuestion {
  ...
  /** Skip when not meaningful (e.g. first-goal bucket with no goals). */
  include: boolean;
  priority: number;
}
...
export function aggregateFirstHalf(events: MatchEventRow[]): FirstHalfStats {
  ...
  for (const ev of events) {
    if (ev.type === 'halfTime') {
      ...
      } else if (ev.scoreHome !== undefined && ev.scoreGuest !== undefined) {
        scoreHome = ev.scoreHome;
        scoreGuest = ev.scoreGuest;
      }
      continue;
    }
    ...
    if (ev.type === 'card' && ev.cardColor === 'yellow') {
      yellowCards += 1;
    }
...
function teamMoreLabel(home: number, guest: number): { correct: string; options: TriviaOption[] } {
  const correct =
    home > guest ? 'HOME' : guest > home ? 'AWAY' : 'EQUAL';
  return {
    correct,
    options: [
      { id: 'HOME', label: 'HOME' },
...
function yellowCardOptions(count: number): { correct: string; options: TriviaOption[] } {
...
function goalsOptions(count: number): { correct: string; options: TriviaOption[] } {
  const correct = count >= 3 ? '3+' : String(count);
  return {
    correct,
    options: [
...
function scoreLabel(h: number, g: number): string {
  ...
  const hi = Math.max(h, g);
  const lo = Math.min(h, g);
  return `${hi}-${lo} ${leader}`;
}

function buildScoreQuestion(stats: FirstHalfStats): GeneratedQuestion {
  ...
  while (decoys.size < 3) {
    decoys.add(scoreLabel(decoys.size, 0));
  }

  const options: TriviaOption[] = [correct, ...Array.from(decoys).slice(0, 3)].map((label, i) => ({
    id: `opt-${i}`,
    label,
  }));
...
function firstGoalBucket(minute: number | null, goals: number): { correct: string; options: TriviaOption[] } {
...
export function buildQuestionPool(stats: FirstHalfStats): GeneratedQuestion[] {
  ...
  return pool.filter((q) => q.include);
}

export function selectThreeQuestions(pool: GeneratedQuestion[]): GeneratedQuestion[] {
...
export function generateTriviaQuestions(events: MatchEventRow[]): GeneratedQuestion[] {
  const stats = aggregateFirstHalf(events);
  const pool = buildQuestionPool(stats);
  return selectThreeQuestions(pool);
}
